...
export default function Album() {
  ...
  const irA = useCallback(
    (dir: number) => {
      setCelebrar(false);
      setPagina((p) => Math.min(selecciones.length - 1, Math.max(0, p + dir)));
      // Scroll al top cuando cambias de equipo
  ...
  // Detectar scroll para cambiar de equipo cuando llegas al final
  const handleScroll = useCallback(
    (e: React.UIEvent<HTMLDivElement>) => {
      ...
      if (atBottom && pagina < selecciones.length - 1) {
        irA(1);
      } else if (atTop && pagina > 0) {
        irA(-1);
      }
    },
    [pagina, selecciones.length, irA]
  );

  // Flechas izq/der del teclado (en PC)
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "ArrowRight") irA(1);
      if (e.key === "ArrowLeft") irA(-1);
      ...
      >
        <div className="max-w-3xl mx-auto px-4 py-6">
          ...
          <div className="flex flex-col items-center mt-5 gap-2">
            ...
            <p className="text-white/50 text-xs">
              Deslizá libremente - continúa scrolleando para el siguiente equipo
            </p>
          </div>
        </div>
      </div>
